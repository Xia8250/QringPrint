 ...
 * 图像抖动 (dithering)。
 *
 * 热敏头是 1-bit 输出,只能打黑或不打。单按阈值二值化会把所有中间灰度
 * 一刀切成纯黑纯白,照片就丢了层次。抖动通过把量化误差扩散到附近像素,
 * 用点阵的疏密在视觉上模拟灰阶。
 *
 * 纯数组运算,不依赖 ImageKit,方便单测。
 */

enum class DitherMode(val value: Int) {
    /** 直接阈值二值化,不扩散误差。线条/文字/二维码用这个最锐利 */
    NONE(0),
    /** Floyd-Steinberg:经典误差扩散,层次最细腻,照片首选 */
    FLOYD_STEINBERG(1),
    /** Atkinson:只扩散 6/8 误差,对比度更高、暗部更干净,早期 Mac 的做法 */
    ATKINSON(2);

    companion object {
        fun fromInt(v: Int): DitherMode = values().firstOrNull { it.value == v } ?: FLOYD_STEINBERG
    }
}
...
val DITHER_OPTIONS: List<DitherOption> = listOf(
    DitherOption(DitherMode.NONE, "无", "硬阈值 · 线条/文字最锐利"),
    DitherOption(DitherMode.FLOYD_STEINBERG, "Floyd", "Floyd-Steinberg · 层次细腻,照片首选"),
    DitherOption(DitherMode.ATKINSON, "Atkinson", "Atkinson · 对比度更高,暗部更干净"),
)
...
data class GrayImage(
    ...
    override fun hashCode(): Int = data.contentHashCode() * 31 + width * 17 + height
}

/**
 * 误差扩散用的中点阈值。
 *
 * 抖动模式固定用 128:误差扩散的前提是量化点落在灰阶中点。
 * 用别的值(比如文字阈 212)会让整张图整体压黑,失去抖动的意义。
 * 只有 NONE 模式才使用调用方传入的 threshold。
 */
private const val DITHER_PIVOT: Int = 128
...
fun ditherToBinary(gray: GrayImage, mode: DitherMode, threshold: Int): ByteArray {
    ...
    if (mode == DitherMode.NONE) {
        for (i in 0 until total) {
            out[i] = if (gray.data[i].toInt() and 0xFF < threshold) 1 else 0
        }
        return out
    }
    ...
    for (i in 0 until total) {
        buffer[i] = (gray.data[i].toInt() and 0xFF).toFloat()
    }
    ...
    for (y in 0 until height) {
        for (x in 0 until width) {
            ...
            val newValue = if (oldValue < DITHER_PIVOT) 0f else 255f
